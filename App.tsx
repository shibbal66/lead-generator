...
import { Lead, PipelineStage, SortField, SortOrder, LeadFile, Owner, Task, Todo, Project, UserSettings, Deal } from './types';
...
import { 
  Plus, Search, Filter, BarChart3, Users, LayoutDashboard, 
  Settings, Loader2, UserPlus, Download, Bell, Send, 
  CheckSquare, ClipboardList, Briefcase, Share2, FolderKanban,
  PieChart, Mail, ShieldCheck, FolderPlus
} from 'lucide-react';
import * as XLSX from 'xlsx';

type ViewType = 'pipeline' | 'analytics' | 'todos' | 'sent_tasks' | 'my_projects' | 'settings' | 'email_settings' | 'user_mgmt';
...
const App: React.FC = () => {
  ...
  const handleExport = () => {
    ...
    const exportData = leads.map(l => ({
      ...
      Projekt: projects.find(p => p.id === l.projectId)?.title || (currentLang === 'de' ? 'Kein Projekt' : 'No Project'),
  ...
  const handleAssignTask = async (text: string, deadline?: string) => {
    ...
    try {
      ...
      sendPushNotification(currentLang === 'de' ? "Neue Aufgabe zugewiesen" : "New Task Assigned", `${taskingOwner.name} ${currentLang === 'de' ? 'hat eine neue Aufgabe erhalten' : 'received a new task'}: ${text}`);
      fetchData();
    } catch (error) {
  ...
  const handleAddComment = async (text: string) => {
    ...
    try {
      ...
      const updatedLeads = leads.map(l => l.id === selectedLead.id ? { ...l, comments: [...l.comments, newComment] } : l);
  ...
  const filteredLeads = useMemo(() => {
    return leads
      .filter(l => {
        ...
        const matchesSearch = `${l.firstName} ${l.lastName}`.toLowerCase().includes(search.toLowerCase()) || l.currentPosition.toLowerCase().includes(search.toLowerCase());
  ...
  const renderMainView = () => {
    if (activeView === 'analytics') return <AnalyticsDashboard deals={deals} leads={leads} owners={owners} projects={projects} lang={currentLang} />;
    ...
    return (
      <div className="flex-1 p-8 overflow-hidden flex flex-col">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="text-sm text-gray-500">{t.pipeline.total}: <span className="font-bold text-gray-900">{filteredLeads.length}</span> Leads</div>
        </div>
        {loading ? <div className="flex-1 flex items-center justify-center"><Loader2 className="animate-spin text-blue-600" size={48} /></div> : <KanbanBoard leads={filteredLeads} onLeadClick={setSelectedLead} onAddDeal={setClosingLead} lang={currentLang} />}
      </div>
    );
  };
  ...
  return (
    <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
      <div className="flex h-screen bg-gray-50 overflow-hidden">
        <aside className="w-64 bg-white border-r border-gray-200 hidden lg:flex flex-col">
          ...
          <nav className="flex-1 px-4 overflow-y-auto space-y-6">
            <div className="space-y-1">
              <button onClick={() => setActiveView('pipeline')} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg font-medium transition-all ${activeView === 'pipeline' ? 'bg-blue-50 text-blue-700' : 'text-gray-500 hover:bg-gray-50'}`}>
                <LayoutDashboard size={20} /> <span>Dashboard</span>
              </button>
              <button onClick={() => setActiveView('analytics')} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg font-medium transition-all ${activeView === 'analytics' ? 'bg-blue-50 text-blue-700' : 'text-gray-500 hover:bg-gray-50'}`}>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between px-3">
                <div className="flex items-center space-x-2 text-gray-400">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest">{currentLang === 'de' ? 'Admin Bereich' : 'Admin Area'}</span>
                </div>
              </div>
              <div className="space-y-1">
                <button onClick={() => setActiveView('user_mgmt')} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg font-medium transition-all ${activeView === 'user_mgmt' ? 'bg-blue-50 text-blue-700' : 'text-gray-500 hover:bg-gray-50'}`}>
                  <ShieldCheck size={18} /> <span className="text-xs font-bold">{t.userMgmt.title}</span>
                </button>
                <button onClick={() => setActiveView('email_settings')} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg font-medium transition-all ${activeView === 'email_settings' ? 'bg-blue-50 text-blue-700' : 'text-gray-500 hover:bg-gray-50'}`}>
                  {/* Fixed: replaced 'lang' with 'currentLang' to fix find name 'lang' error */}
                  <Mail size={18} /> <span className="text-xs font-bold">{currentLang === 'de' ? 'E-Mail Konfig' : 'Email Config'}</span>
                </button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <button onClick={() => setActiveView('todos')} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg transition-all ${activeView === 'todos' ? 'bg-blue-50 text-blue-700' : 'text-gray-500 hover:bg-gray-50'}`}>
                  <div className="flex items-center space-x-3"><CheckSquare size={18} /> <span className="text-xs font-bold">{t.sidebar.myTodos}</span></div>
                </button>
                <button onClick={() => setActiveView('my_projects')} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg transition-all ${activeView === 'my_projects' ? 'bg-blue-50 text-blue-700' : 'text-gray-500 hover:bg-gray-50'}`}>
                  <div className="flex items-center space-x-3"><FolderKanban size={18} /> <span className="text-xs font-bold">{t.sidebar.myProjects}</span></div>
                </button>
                <button onClick={() => setActiveView('settings')} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg font-medium transition-all ${activeView === 'settings' ? 'bg-blue-50 text-blue-700' : 'text-gray-500 hover:bg-gray-50'}`}>
        ...
        <main className="flex-1 flex flex-col min-w-0">
          <header className="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between sticky top-0 z-40">
            ...
            <div className="flex items-center space-x-3 ml-6">
              ...
              >
                {/* Fixed: replaced 'lang' with 'currentLang' to fix find name 'lang' error */}
                <FolderPlus size={18} className="mr-2 text-indigo-600" /> {currentLang === 'de' ? 'Projekt anlegen' : 'Create Project'}
              </button>
        ...
        <LeadDetailDrawer lead={selectedLead} projects={projects} onClose={() => setSelectedLead(null)} onUpdate={handleUpdateLead} onAddComment={handleAddComment} lang={currentLang} />
        {isModalOpen && <LeadModal owners={owners} onClose={() => setIsModalOpen(false)} onSave={handleCreateLead} lang={currentLang} />}
        {isProjectModalOpen && <ProjectModal leads={leads} owners={owners} lang={currentLang} onClose={() => setIsProjectModalOpen(false)} onSave={handleCreateProject} />}
        {isTrashModalOpen && <TrashModal leads={leads.filter(l => l.pipelineStage === PipelineStage.TRASH)} lang={currentLang} onClose={() => setIsTrashModalOpen(false)} onRestore={id => api.updateLead(id, { pipelineStage: PipelineStage.IDENTIFIED }).then(fetchData)} onPermanentDelete={id => api.saveLeads(leads.filter(l => l.id !== id)).then(fetchData)} />}
        {taskingOwner && <TaskModal owner={taskingOwner} onClose={() => setTaskingOwner(null)} onAssign={handleAssignTask} lang={currentLang} />}
        {closingLead && <DealModal lead={closingLead} projects={projects} lang={currentLang} onClose={() => setClosingLead(null)} onSave={handleSaveDeal} />}
      </div>
    </DndContext>
  );
};
